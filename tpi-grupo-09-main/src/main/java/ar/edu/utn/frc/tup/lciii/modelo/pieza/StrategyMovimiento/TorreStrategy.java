...
public class TorreStrategy implements MovimientoStrategy {
    @Override
    public String moverPieza(int colOrigen, int filaOrigen, int filaDestino, int colDestino) {
        ...
        if (lstMovimientosPermitidos.contains(casillaDestino)){
            ...
            if (colorPieza==Color.BLANCO){
                tablero[filaDestino][colDestino].SetPieza(new Pieza(EnumPieza.TORRE_B));
                tablero[filaOrigen][colOrigen].SetPieza(null);
            }
            else if (colorPieza==Color.NEGRO){
                tablero[filaDestino][colDestino].SetPieza(new Pieza(EnumPieza.TORRE_N));
                tablero[filaOrigen][colOrigen].SetPieza(null);
            }
...
//                                             MAPEO EN CRUZ
    ...
    public String anotarJugada(int filaOrigen, int colOrigen, int filaDestino, int colDestino,  boolean comioPieza) {
        ...
        if(comioPieza){
            cadenaResultado= EnumPieza.TORRE_N.getNotacion() + "x"+ MovimientoPieza.convertirNumeroALetra(colDestino)+(filaDestino+1);
        }else{
            cadenaResultado= EnumPieza.TORRE_N.getNotacion() + MovimientoPieza.convertirNumeroALetra(colDestino) + (filaDestino+1);
        }
