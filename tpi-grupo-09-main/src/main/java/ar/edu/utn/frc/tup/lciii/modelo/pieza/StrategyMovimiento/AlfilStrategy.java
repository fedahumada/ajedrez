...
public class AlfilStrategy implements MovimientoStrategy {
    @Override
    public String moverPieza(int colOrigen, int filaOrigen, int filaDestino, int colDestino) {
        ...
        if (lstMovimientosPermitidos.contains(casillaDestino)){
            ...
            if (colorPieza==Color.BLANCO){
                tablero[filaDestino][colDestino].SetPieza(new Pieza(EnumPieza.ALFIL_B));
                tablero[filaOrigen][colOrigen].SetPieza(null);
            }
            else {
                tablero[filaDestino][colDestino].SetPieza(new Pieza(EnumPieza.ALFIL_N));
                tablero[filaOrigen][colOrigen].SetPieza(null);
            }
...
//                                          MAPEO EN EQUIS
    ...
    public String anotarJugada(int filaOrigen, int colOrigen, int filaDestino, int colDestino,  boolean comioPieza) {
       String cadenaResultado;
        ...
        if(comioPieza){
            cadenaResultado= EnumPieza.ALFIL_B.getNotacion() + "x"+ MovimientoPieza.convertirNumeroALetra(colDestino)+(filaDestino+1);
        }else{
            cadenaResultado= EnumPieza.ALFIL_B.getNotacion() + MovimientoPieza.convertirNumeroALetra(colDestino) + (filaDestino+1);
        }

        return cadenaResultado;
    }
    @Override
    public boolean esJaqueAtaque(Casilla casilla, Color colorPieza){
        ...
        List<Casilla> lstValidarJaqueAtaque = movimientosValidos(casilla, colorPieza);
